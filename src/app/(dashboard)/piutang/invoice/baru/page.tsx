...
export default function BuatInvoicePage() {
  ...
  const handleSubmit = async (status: string) => {
    ...
    try {
      const res = await fetch('/api/invoices', {
        ...
        body: JSON.stringify({
          customerId, date, dueDate, notes, items, subtotal, ppnTotal, total, status
        }),
      });
      if (res.ok) {
        router.push('/piutang/invoice');
      }
    } catch (error) {
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold text-gray-800 mb-6">Buat Invoice Baru</h1>
      ...
      <div className="bg-white rounded-md shadow p-6 space-y-6">
        ...
        <div>
          <h3 className="font-semibold text-lg mb-2">Item Invoice</h3>
          <Table>
            <TableHeader>
              <TableRow>
